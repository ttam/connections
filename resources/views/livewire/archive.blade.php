    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @foreach($puzzles as $puzzle)
            <a href="{{ route('game.play', $puzzle->id) }}" wire:navigate
               class="block p-6 bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md hover:border-black transition-all cursor-pointer group">
                <h2 class="text-xl font-bold text-gray-900 group-hover:underline decoration-2 underline-offset-4">
                    {{ $puzzle->title }}
                </h2>
                <p class="text-sm text-gray-500 mt-2">
                    {{ $puzzle->play_date->format('M j, Y') }}
                </p>
                <div class="mt-4 text-sm font-semibold text-black flex items-center gap-1">
                    Play now
